  ...
  function fillNames(sheet) {
    ...
    sheet.nameSource = paper.read
      ...
      : paper.bad
        ...
        // 이름 칸 버블을 3.2×2.6mm 두 줄로 키우면서 자리가 달라졌다. 그 전에 뽑아 둔
        // 종이는 이름만 못 읽는다 — 숫자는 전부 그대로 읽힌다(붙박이 시험이 증거다).
        // 그러니 "못 읽었다"가 아니라 "고르시면 됩니다"라고 말해야 한다.
        : { ok: false, why: "이름 칸을 키우기 전에 뽑은 종이입니다 — 숫자는 다 읽었으니 " +
              "아래에서 줄마다 선수만 골라주세요. 다음 인쇄부터는 이름도 자동으로 읽힙니다." };
    return hit > 0;
  }
