 ...
 */
// 이름 칸 일곱 개를 0/1 로 가른다 — **종이보다 10% 넘게 어두우면 칠한 것**.
//
// 여기까지 오는 데 세 번을 헤맸다. 기록으로 남긴다.
//
//  1) 손마킹용 문턱값(0.62)을 그대로 썼다. 이 칸들은 사람이 칠하는 게 아니라
//     인쇄되어 나오고 2.1×1.9mm 로 작아서 손마킹만큼 진하지 않다. 인쇄된 칸 무리
//     한가운데를 갈라 홀짝 검사가 줄줄이 터졌다.
//  2) 그 줄 안에서 "가장 많이 벌어진 곳"을 찾아 잘랐다. 밝은 쪽 꼬리가 길어서
//     (이웃이 어두우면 2.8 까지 간다) 엉뚱한 데를 골랐다.
//  3) 자를 만한 자리를 전부 시험하고 홀짝·명단으로 거른 뒤 어두운 칸이 가장 많은
//     답을 골랐다. 이번엔 "1.2배 이상 벌어진 자리만 후보" 라는 조건 하나에 걸렸다 —
//     같은 사진이라도 브라우저가 다르면 어둡기가 0.03~0.05 움직이는데, 1.23배이던
//     것이 1.15배가 되자 **정답이 후보 목록에 오르지도 못했고**, 어두운 칸 둘짜리
//     함정(손걸 = 0000101)만 남아 그게 뽑혔다.
//
// 세 번 다 "어디서 자를까"를 사진마다 새로 정하려 한 게 화근이었다. 그런데 지금까지
// 실제로 잰 열세 줄(혼A 여섯 · 혼B 일곱)을 모아 놓고 보니 자를 자리는 처음부터
// 하나뿐이었다:
//
//     칠해진 칸 중 가장 밝은 것   0.88
//     빈 칸 중 가장 어두운 것     0.93
//
// 이 값은 이미 옆 칸 종이 색으로 나눈 뒤의 값이라(채널마다 따로), 0.9 는 임의의
// 숫자가 아니라 **"종이보다 10% 넘게 어둡다"** 는 뜻이다. 종이가 누렇든 조명이
// 어둡든 그대로 성립한다. 열세 줄 전부 이 한 자리로 맞는다.
//
// 못 가르는 줄은 그냥 못 읽는다고 한다 — 신윤호 줄은 일곱 칸 중 둘이 빈 칸보다
// 밝게 찍혀 **어떤 문턱값으로도** 가를 수 없다. 그건 인쇄 문제지 판독기 문제가 아니고,
// 엉뚱한 이름을 내놓느니 사람이 고르게 두는 게 낫다.
const NAME_CUT = 0.9;
...
export function readSheetRoster(readings, roster) {
  ...
  for (let p = 0; p < PLAYER_ROWS; p++) {
    ...
    // 종이보다 10% 넘게 어두우면 칠한 것으로 본다. 손마킹용 문턱값은 쓰지 않는다 —
    // 그 길이 용원식을 손걸로 만들었다.
    const idx = rowCodeValue(ratios.map((v) => (v < NAME_CUT ? 1 : 0)));
    const who = idx != null && idx >= 0 ? roster?.[idx] : null;
    if (!who) {
      // 홀짝 검사에 걸렸는데 일곱 칸이 다 밝으면 그냥 빈 줄이다(예비 줄).
      rows.push(null);
      if (idx !== -1 && Math.min(...ratios) < NAME_CUT) bad++;
      continue;
    }
